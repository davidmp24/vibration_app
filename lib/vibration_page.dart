...
class _VibrationPageState extends State<VibrationPage> with WidgetsBindingObserver {
  ...
  Future<void> _sendVibrationCommand({
    List<int>? pattern,
    int? vibrationCount,
  }) async {
    if (_channelId == null) {
    ...
    } else if (vibrationCount != null) {
      command['vibrationCount'] = vibrationCount;
      command['vibrationDuration'] = vibrationDuration;
      command['vibrationSpacing'] = vibrationSpacing;
    }

    await _firestore.collection('channels').doc(_channelId).set(command);
  }
  ...
  Future<void> _sendContinuousVibration(bool start) async {
    ...
    await _sendVibrationCommand(pattern: pattern);
  }
  ...
  void _vibrateCustomPattern() async {
    ...
    await _sendVibrationCommand(vibrationCount: vibrationCount);
  }
  ...
  // ALTERAÇÃO: Os botões de atalho agora enviam o comando diretamente.
  Widget _buildPresetButton(String label) {
    return ElevatedButton(
      onPressed: () {
        final int? vibrationCount = _presetValues[label];
        if (vibrationCount != null) {
          _sendVibrationCommand(vibrationCount: vibrationCount);
        }
      },
